    ...
    <div className="group bg-white border border-brand-border overflow-hidden hover:shadow-[0_4px_24px_rgba(215,118,39,0.12)] transition-shadow duration-300">
      <a href={`/en/product/${product.slug}`} className="block">
        <div className="relative aspect-square overflow-hidden bg-brand-surface">
          {img ? (
            <Image src={img.src} alt={img.alt || product.name} fill sizes="(max-width: 640px) 50vw, (max-width: 1024px) 33vw, 25vw" className="object-cover transition-transform duration-500 group-hover:scale-105" />
          ) : (
        ...
        <div className="px-4 pt-4 pb-1">
          {cat && <p className="font-body text-brand-orange text-[10px] uppercase tracking-widest mb-1">{EN_CATEGORY_NAMES[cat.slug] ?? cat.name}</p>}
          <h3 className="font-heading text-sm lg:text-base uppercase tracking-wide text-brand-dark leading-tight">{product.name}</h3>
        </div>
      </a>
...
export default async function EnCatalogPage({
        ...
        <main>
          <section className="relative bg-brand-footer py-20 lg:py-28 overflow-hidden">
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-4 md:px-8 lg:px-16 xl:px-24">
              ...
              <p className="font-body text-brand-orange text-xs uppercase tracking-[0.25em] mb-4">Search results</p>
              <h1 className="font-heading text-white text-4xl lg:text-6xl uppercase tracking-wide leading-tight">&ldquo;{query}&rdquo;</h1>
            ...
            <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-16 xl:px-24">
              {results.length === 0 ? (
                <div className="text-center py-20">
                  ...
                  <a href="/en/catalog" className="font-body text-xs uppercase tracking-widest text-brand-orange border-b border-brand-orange/40 pb-0.5 hover:border-brand-orange transition-colors">
                    View full catalog →
                  </a>
                </div>
              ) : (
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-5">
                  {results.map((product) => <SearchResultCard key={product.slug} product={product} />)}
                </div>
              )}
            </div>
          </section>
  ...
  const categoriesWithProducts: CategoryWithProduct[] = await Promise.all(
    categories.map(async (cat) => {
      ...
      try {
        const [product] = await getProducts({ category: cat.id, per_page: 1, orderby: "date", order: "desc" });
